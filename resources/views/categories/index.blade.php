...
<div class="conten-wrapper">
    <section class="content container-fluid">
    	...
    	<div class="container">
			...
			<hr>
			<a href="{{ route('categories.create') }}" type="button" class="btn btn-primary">Tạo ví mới</a>
			<hr>
            <div class="row">
                ...
                <table id="table1" class="table table-hover table-striped">
                    ...
                    <tbody>
                        @foreach($categories as $key => $wallet)
                            <tr>
                                <td>{{ $key + 1 }}</td>
                                <td>{{ $wallet->name }}</td>
                                <td>{{ $wallet->ssid }}</td>
                                <td>{{ number_format($wallet->money). ' vnđ' }}</td>
                                <td><a href="{{ route('categories.edit', $wallet->id) }}"><span class="fa fa-edit"></span> Sửa</a></td>
                                <td><form action="{{ route('categories.destroy', $wallet->id )}}" method="POST">
                                @method('DELETE')
                                @csrf
